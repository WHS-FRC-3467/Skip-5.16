 ...
 *
 * <p>{@code HubState} uses {@link edu.wpi.first.wpilibj.DriverStation#getMatchTime()} together with
 ...
 *
 * <p>The class exposes two {@link edu.wpi.first.wpilibj2.command.button.Trigger} instances:
 *
 * <ul>
 *   <li>{@link #hubChange} – becomes active shortly before the next scheduled hub change, so
 *       subsystems or commands can prepare for the transition.
 *   <li>{@link #hubActive} – indicates when the alliance hub tracked by this class is currently the
 *       same as the robot's {@link DriverStation#getAlliance()}.
 * </ul>
 *
...
public class HubState {
    ...
    private LoggedTrigger hubChange =
            new LoggedTrigger(
                    "Hub/Hub is about to change (within " + SECONDS_BEFORE + " seconds)",
                    () -> isCloseToSwitching());
    ...
    /**
     * Meant to be called regularly. Checks which hub is active and stores it in {@link
     * #activeAlliance}
     */
